...
#define SSH_DBG(fmt, sock, ...)                                                                                      \
	lrtr_dbg("SSH Transport(%s@%s:%u): " fmt, (sock)->config.username, (sock)->config.host, (sock)->config.port, \
		 ##__VA_ARGS__)
#define SSH_DBG1(a, sock) \
	lrtr_dbg("SSH Transport(%s@%s:%u): " a, (sock)->config.username, (sock)->config.host, (sock)->config.port)
...
int tr_ssh_open(void *socket)
{
	...
	if ((config->server_hostkey_path != NULL) &&
	    (ssh_is_server_known(ssh_socket->session) != SSH_SERVER_KNOWN_OK)) {
