...
/**
 * @brief Removes prefix PDU from the @p pfx_table if flag field == ADD, adds PDU to the @p pfx_table if flag
 * field == REMOVE.
 */
...
/**
 * @brief Reverts all changes made to the given @p pfx_table based on the IPv4 and IPv6 PDUs supplied.
 */
...
/**
 * @brief Removes @c router_key from the @p spki_table if flag field == ADD,
 * ADDs @c router_key PDU to the @p spki_table if flag field == REMOVE.
 */
...
/**
 * @brief Reverts all changes made to the given @p spki_table based on the router key PDUs supplied.
 */
...
/**
 * @brief Updates @p pfx_table and @p spki_table with given pdus, undoes changes made if update fails.
 */
static int rtr_sync_update_tables(struct rtr_socket *rtr_socket, struct pfx_table *pfx_table,
				  struct spki_table *spki_table, struct pdu_ipv4 *ipv4_pdus,
				  const unsigned int ipv4_pdu_count, struct pdu_ipv6 *ipv6_pdus,
				  const unsigned int ipv6_pdu_count, struct pdu_router_key *router_key_pdus,
				  const unsigned int router_key_pdu_count, struct pdu_end_of_data_v0 *eod_pdu)
{
	...
	for (size_t i = 0; i < ipv4_pdu_count; i++) {
		if (rtr_update_pfx_table(rtr_socket, pfx_table, &(ipv4_pdus[i])) == RTR_ERROR) {
			...
			// update failed, so try to undo all changes made to the pfx_table
	...
	if (update_res != RTR_ERROR) {
		...
		for (size_t i = 0; i < router_key_pdu_count; i++) {
			if (rtr_update_spki_table(rtr_socket, spki_table, &(router_key_pdus[i])) == RTR_ERROR) {
				...
				// update failed, so try to undo all changes made to the spki_table
	...
	RTR_DBG("Sync successful, received %u Prefix PDUs, %u Router Key PDUs, session_id: %u, SN: %u",
		(ipv4_pdu_count + ipv6_pdu_count), router_key_pdu_count, rtr_socket->session_id,
...
/**
 * @brief Handles EOD PDU contents.
 */
		...
		case EOD:
			...
			if (rtr_socket->is_resetting) {
				...
				retval = rtr_sync_update_tables(rtr_socket, pfx_shadow_table, spki_shadow_table,
								...
								router_key_pdus_nindex, eod_pdu);
...
cleanup:
				...
				retval = rtr_sync_update_tables(rtr_socket, rtr_socket->pfx_table,
								rtr_socket->spki_table, ipv4_pdus, ipv4_pdus_nindex,
