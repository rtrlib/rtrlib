				...
				struct spki_table *table)
{
	...
	for (unsigned int offset = 0, next_offset = 0;
	     offset <= get_stream_size(s) && retval == RTR_BGPSEC_VALID;
	     offset += next_offset) {
		if (tmp_sig->next)
		...
		next_offset = tmp_sig_len +
			      SKI_SIZE +
			      sizeof(tmp_sig->sig_len) +
		 ...
		 */
		uint16_t len = (uint16_t)(get_stream_size(s) - offset);

		uint8_t *curr = lrtr_malloc(len);

		read_stream_at(curr, s, offset, len);

		retval = hash_byte_sequence(curr,
					    len,
					    data->alg,
					    &hash_result);
		...
		for (unsigned int j = 0; j < router_keys_len; j++) {
			...
			if (data->alg == RTR_BGPSEC_ALGORITHM_SUITE_1) {
				retval = validate_signature(hash_result,
							    tmp_sig,
							    &tmp_key[j]);
			} else {
			  ...
			  struct rtr_signature_seg **new_signature)
{
	...
	//TODO: error case not covered by unit test.
	*new_signature = rtr_bgpsec_new_signature_seg(NULL, req_sig_size, NULL);
	...
	retval = sign_byte_sequence(hash_result,
				    priv_key,
				    data->alg,
				    *new_signature);
...
{
	if (!new_seg || !new_seg->signature ||
	    !new_seg->sig_len || ski_is_empty(new_seg->ski)) {
		return RTR_BGPSEC_ERROR;
	}
							   ...
							   uint32_t asn)
{
	struct rtr_secure_path_seg *seg =
		lrtr_malloc(sizeof(struct rtr_secure_path_seg));
