 ...
 * records are stored in an @c aspa_array dynamically ordering the records inside in order to make searching faster. The table
 * stores each of these @c aspa_array references alongside a reference to the beforementioned @c rtr_socket inside a linked
 * list.
 * Adding or removing records, or purging all records to any given socket would involve performing a lookup on the linked list. This
 * is avoided by storing a reference to the corresponding @c aspa_array in every @c rtr_socket . All possible operations
 ...
 * mechanism is used since there is only one "global" @c aspa_table anyway -- namely the one stored in each @c rtr_socket .
 *
 * @{
 */
