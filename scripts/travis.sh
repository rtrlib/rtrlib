...
function checkpatch {
	...
	run_command scripts/checkpatch.pl --ignore FILE_PATH_CHANGES,PREFER_KERNEL_TYPES,CONST_STRUCT,OPEN_BRACE,SPDX_LICENSE_TAG,OPEN_ENDED_LINE,UNNECESSARY_PARENTHESES,PREFER_PRINTF,GLOBAL_INITIALISERS --terse --no-tree --strict --show-types /tmp/patch
	ret=$?
	if [ $ret != 0 ]; then
		cat -n /tmp/patch
	fi
